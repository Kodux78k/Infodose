...
/* ===========================================================
   UNO • HUB — Consolidado "Plugar e Brincar"
   Patches aplicados:
   1) Registry + Bundles (starter/full/loja) com enable/disable
   2) Anti-duplicação do catálogo (RAW_BASE_APPS + renderAppsFromRegistry)
   3) Import de "app pelado" (HTML com <!-- app-manifest: {...} -->)
   4) Fallback de ícones (icons/*.svg → .png → inline)
   5) Service Worker resiliente (service-worker.js ⇄ sw.js)
   6) Console helper: window.AppBundles
   ----------------------------------------------------------- */

/* ===== Registry (apps instaláveis) =================================== */
(function registryModule(){
  if (window.__UNO_REGISTRY_PATCHED__) return; // idempotente
  window.__UNO_REGISTRY_PATCHED__ = true;

  const REGISTRY_KEY = 'uno:apps.registry';
  const BUNDLE_KEY   = 'uno:apps.bundle'; // starter | full | loja

  const DEFAULT_REGISTRY = [
    { key:'apps',  title:'Apps',   icon:'folder-open', entry:'about:blank',
      capabilities:['openInside','stack','favorites'], bundle:'core', enabled:true, source:'registry' },
    { key:'brain', title:'Usuário',icon:'user',        entry:'about:blank',
      capabilities:['panel','openInside'],             bundle:'core', enabled:true, source:'registry' },
    { key:'chat',  title:'Chat',   icon:'comments',    entry:'about:blank',
      capabilities:['openInside','stack'],             bundle:'core', enabled:true, source:'registry' },
  ];

  const BUNDLES = {
    starter: ['apps','brain','chat'],
    full:    ['apps','brain','chat'], // expanda quando quiser
    loja:    ['apps','brain','chat'], // catálogo/demos adicionais
  };

  function loadRegistry(){
    try {
      const raw = localStorage.getItem(REGISTRY_KEY);
      if(!raw) return JSON.parse(JSON.stringify(DEFAULT_REGISTRY));
      const arr = JSON.parse(raw);
      arr.forEach(a => a.source = a.source || 'registry');
      return arr;
    } catch { return JSON.parse(JSON.stringify(DEFAULT_REGISTRY)); }
  }
  function saveRegistry(arr){
    try { localStorage.setItem(REGISTRY_KEY, JSON.stringify(arr)); } catch {}
  }
  function currentBundle(){
    let b = localStorage.getItem(BUNDLE_KEY);
    if(!b){ b = 'starter'; localStorage.setItem(BUNDLE_KEY, b); }
    return b;
  }
  function applyBundle(reg){
    const enabledSet = new Set(BUNDLES[currentBundle()] || []);
    reg.forEach(a => { a.enabled = enabledSet.has(a.key); });
    saveRegistry(reg);
  }
  function registryToRawApps(reg){
    return (reg||[]).filter(a => a.enabled).map(a => ({
      key:  a.key,
      title:a.title,
      desc: a.desc || '',
      url:  a.entry || 'about:blank',
      icon: a.icon || 'atlas',
      source:'registry',
      tags: ['registry', a.bundle || 'core']
    }));
  }

  async function importAppFromFile(file){
    const text = await file.text();
    const m = text.match(/<!--\s*app-manifest:\s*([\s\S]*?)\s*-->/i);
    if(!m) throw new Error('Manifesto não encontrado no HTML');
    const manifest = JSON.parse(m[1]);
    if(!manifest.key) throw new Error('Manifesto sem "key"');

    const blob = new Blob([text], { type: 'text/html' });
    const url  = URL.createObjectURL(blob);

    const reg  = loadRegistry();
    const idx  = reg.findIndex(a => a.key === manifest.key);
    const item = {
      key: manifest.key,
      title: manifest.title || manifest.key,
      icon: manifest.icon  || 'bolt',
      entry: url,
      capabilities: manifest.capabilities || ['openInside','stack'],
      bundle: 'loja',
      enabled: true,
      source: 'registry'
    };
    if(idx >= 0) reg[idx] = { ...reg[idx], ...item }; else reg.push(item);
    saveRegistry(reg);

    try { renderAppsFromRegistry(); } catch {}
    return item.key;
  }

  function enableApp(key, on=true){
    const reg = loadRegistry();
    const app = reg.find(a => a.key === key);
    if(app){ app.enabled = !!on; saveRegistry(reg); try{ renderAppsFromRegistry(); }catch{} }
  }
  function disableApp(key){ enableApp(key, false); }

  // Snapshot imutável dos apps embutidos
  window.RAW_BASE_APPS = (typeof RAW !== 'undefined' && Array.isArray(RAW.apps)) ? RAW.apps.slice() : [];

  window.renderAppsFromRegistry = function(){
    try{
      const reg = loadRegistry();
      const embedded = registryToRawApps(reg);
      const base = (Array.isArray(window.RAW_BASE_APPS) ? window.RAW_BASE_APPS : []).slice();

      RAW.apps = embedded.concat(base);

      if(!localStorage.getItem(REGISTRY_KEY+':booted')){
        applyBundle(reg);
        localStorage.setItem(REGISTRY_KEY+':booted','1');
        RAW.apps = registryToRawApps(loadRegistry()).concat(base);
      }
      try { renderApps(); } catch(e){ console.warn('renderApps falhou:', e); }
    }catch(e){ console.warn('renderAppsFromRegistry falhou:', e); }
  };

  // Hook no DOM ready: captura base e re-render com registry
  document.addEventListener('DOMContentLoaded', ()=>{
    try{
      if(typeof RAW !== 'undefined' && Array.isArray(RAW.apps)){
        window.RAW_BASE_APPS = RAW.apps.slice();
      }
    }catch{}
    try{ window.renderAppsFromRegistry(); }catch{}
  });

  // Expor console helpers
  window.AppBundles = {
    set(bundle){ localStorage.setItem(BUNDLE_KEY, bundle); applyBundle(loadRegistry()); window.renderAppsFromRegistry(); },
    get(){ return currentBundle(); },
    enable: enableApp,
    disable: disableApp
  };

  // Rewire do import: combina app pelado + locais + apps.json
  document.addEventListener('DOMContentLoaded', ()=>{
    const btn   = document.getElementById('btnImport');
    const input = document.getElementById('fileLocal');
    if(!btn || !input) return;
    ...
    btn.onclick = async () => {
      ...
      for(const f of files){
        ...
        // 1) apps.json substitui catálogo embutido/base
        if(/\.(json)$/i.test(f.name)){
          try{
            const obj  = JSON.parse(text);
            const apps = Array.isArray(obj.apps) ? obj.apps : (Array.isArray(obj) ? obj : []);
            window.RAW_BASE_APPS = apps.slice();
            RAW.apps = apps.slice();
            window.renderAppsFromRegistry();
            if(typeof toast==='function') toast('apps.json local carregado','ok');
          }catch(err){
            console.error(err);
            if(typeof toast==='function') toast('Erro ao ler apps.json','err');
          }
          continue;
        }

        // 2) HTML com mini-manifesto → app pelado (registry)
        if(/\.(html?|htm)$/i.test(f.name) && /<!--\s*app-manifest:/i.test(text)){
          try{
            const key = await importAppFromFile(new File([text], f.name, {type:'text/html'}));
            if(typeof toast==='function') toast('App instalado: '+key,'ok');
            continue; // não salvar como local
          }catch(err){
            console.warn('Import app pelado falhou:', err);
          }
        }

        // 3) fallback: tratar como HTML local
        if(/\.(html?|htm)$/i.test(f.name)){
          localsToAdd.push({ id:'l_'+Math.random().toString(36).slice(2), name:f.name.replace(/\.(html?|txt)$/i,''), html:text, ts:Date.now() });
        }
      }

      if(localsToAdd.length){
        const cur = JSON.parse(localStorage.getItem('infodose:locals:v1') || '[]');
        localsToAdd.forEach(x => cur.unshift(x));
        localStorage.setItem('infodose:locals:v1', JSON.stringify(cur));
        try{ renderApps(); }catch{}
        if(typeof toast==='function') toast('HTMLs locais adicionados','ok');
      }
      input.value = '';
    };
  });
...
/* ===== Fallback de ícones (icons/*.svg → .png → inline) ================= */
(function patchIcons(){
  if (window.__UNO_ICON_PATCHED__) return;
  window.__UNO_ICON_PATCHED__ = true;

  window.resolveIconSrc = function(icon){
    if(!icon) return svgIcon('atlas');
    if(/^data:/.test(icon) || /[\/:]/.test(icon)) return icon; // já é caminho/URL
    if(/^(atlas|nova|vitalis|pulse|artemis|serena|kaos|genus|lumine|rhea|solus|aion|local|audio|bolt|download|grid|home|json|logo-capsule|logo-seed-split|pause|play|upload|user|sprites)$/.test(icon)){
      return svgIcon(icon);
    }
    return 'icons/' + icon.replace(/\.(svg|png|jpg|jpeg|gif)$/i,'') + '.svg';
  };

  const oldAppIconFor = window.appIconFor;
  window.appIconFor = function(a){
    try { return resolveIconSrc(a && a.icon); }
    catch(e){ return oldAppIconFor ? oldAppIconFor(a) : svgIcon('atlas'); }
  };

  const oldCardApp = window.cardApp;
  if(typeof oldCardApp === 'function'){
    window.cardApp = function(a){
      const el = oldCardApp(a);
      try{
        const img = el.querySelector('.app-icon img');
        if(img){
          img.src = resolveIconSrc(a && a.icon);
          img.onerror = () => {
            const tried = img.getAttribute('data-fallback') || 'svg';
            if (tried === 'svg') {
              img.setAttribute('data-fallback','png');
              img.src = String(img.src).replace(/\.svg(\?.*)?$/i, '.png$1');
            } else {
              img.removeAttribute('onerror');
              img.src = svgIcon('atlas');
            }
          };
        }
      }catch(e){}
...
/* ===== Service Worker resiliente (service-worker.js ⇄ sw.js) ============ */
(function registerServiceWorkerResilient(){
  if (location.protocol !== 'https:' && location.hostname !== 'localhost') return;
  if (!('serviceWorker' in navigator)) return;
  window.addEventListener('load', async function(){
    try {
      const regs = await navigator.serviceWorker.getRegistrations();
      if (Array.isArray(regs) && regs.length > 0) return; // já há SW
    } catch {}

    const candidates = ['service-worker.js','sw.js','./service-worker.js','./sw.js'];
    for (const url of candidates){
      try{
        const r = await navigator.serviceWorker.register(url);
        console.log('[PWA] SW registrado (resiliente):', r.scope);
        return;
